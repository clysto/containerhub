...
func (c *sshClient) wsWrite() error {
	...
	for {
		...
		if n > 0 {
			...
			if err := c.conn.WriteMessage(websocket.BinaryMessage, data[:n]); err != nil {
...
func (c *sshClient) bridgeWSAndSSH() {
	...
	privateKey, err := ssh.ParsePrivateKey(global.Config.SSH.GlobalPrivkey)
	if err != nil {
		log.Println("bridgeWSAndSSH: ssh.ParsePrivateKey:", err)
		return
	}

	cert, _, _, _, err := ssh.ParseAuthorizedKey(global.Config.SSH.GlobalCert)
	if err != nil {
		log.Println("bridgeWSAndSSH: ssh.ParseAuthorizedKey:", err)
		return
	}

	certSigner, err := ssh.NewCertSigner(cert.(*ssh.Certificate), privateKey)
	if err != nil {
		log.Println("bridgeWSAndSSH: ssh.NewCertSigner:", err)
		return
	}

	auth := ssh.PublicKeys(privateKey, certSigner)
...
func SSHHandler(ctx *gin.Context) {
	...
	sshHandler := &sshHandler{
		addr: container.Labels["containerhub-hostname"] + ":22",
		user: "user",
	}

	sshHandler.webSocket(ctx.Writer, ctx.Request)
}
